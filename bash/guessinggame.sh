...
I will pick a secret number from 1 to 100 and you have to guess it.
...
secretnumber=$(($RANDOM % 100 +1)) # save our secret number to compare later
userguess=0
...
while [ $userguess != $secretnumber ]; do # loop around until they get it right
  read -p "Give me a number from 1 to 100: " userguess # ask for a guess
  
  while [ -z "$userguess" ];do
    read -p "No blank. Please give some input from 1 to 100: " userguess
  done
  while [ "$userguess" -lt 1 -o "$userguess" -gt 100 ];do
    read -p "please give some input between 1 to 100: " userguess
  done
...
if [ -z "$userguess" ]
  then
  read -p "please give some input from 1 to 100: " userguess
fi
if [ "$userguess" -lt 1 -o "$userguess" -gt 100 ]
  then
  read -p "please give some input between 1 to 100: " userguess
fi
'
